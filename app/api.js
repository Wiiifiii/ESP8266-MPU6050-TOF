...
function setStartBase(v) {
  START_BASE = v;
}
function setFinishBase(v) {
  FINISH_BASE = v;
}
function getStartBase() {
  return START_BASE;
}
function getFinishBase() {
  return FINISH_BASE;
}

// Real implementations gathered in one object
/**
 * Fetch live car data from 192.168.4.1.
 * @returns {Promise<{data: {speed:number, accel:number, pitch?:number, roll?:number, ax?:number, ay?:number, az?:number, distance?:number, sampleHz?:number, rssi?:number}}>}
 */
const real = {
  getCar: () => carApi.get('/data'),
  /**
   * Get Start unit status.
   * @returns {Promise<{data:{distanceMm?:number, ready?:boolean}}>}
   */
  getStart: () => axios.get(`${START_BASE}/status`, { timeout: 1200 }),
  /**
   * Get Finish unit status.
   * @returns {Promise<{data:{distanceMm?:number, finished?:boolean}}>}
   */
...
export const startLearnForward = () =>
  axios.get('http://192.168.4.1/learn_forward/start', { timeout: 1500 });
