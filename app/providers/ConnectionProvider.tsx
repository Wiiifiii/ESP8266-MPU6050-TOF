...
export function ConnectionProvider({ children }: { children: React.ReactNode }) {
  const [car, setCar] = useState<UnitState>({
    ...defaultState,
    unit: 'car',
    base: 'http://192.168.4.1',
  });
  const [start, setStart] = useState<UnitState>({
    ...defaultState,
    unit: 'start',
    base: getStartBase?.() ?? 'http://192.168.4.2',
  });
  const [finish, setFinish] = useState<UnitState>({
    ...defaultState,
    unit: 'finish',
    base: getFinishBase?.() ?? 'http://192.168.4.3',
  });
  ...
  const pollOnce = async () => {
    ...
    try {
      ...
      setCar((prev) => ({
        ...prev,
        lastErr: now,
        errorsInRow: (prev.errorsInRow || 0) + 1,
        status: computeStatus(prev),
      }));
    }
