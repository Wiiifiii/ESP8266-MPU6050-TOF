  ...
  useEffect(() => {
    ...
    const loop = async () => {
      try {
        ...
        const now = Date.now();
        const ax = Number(car.ax), ay = Number(car.ay);
        ...
        if ((!Number.isFinite(spd) || spd < 0) && Number.isFinite(dist) && prev && Number.isFinite(prev.distance)) {
          ...
          if (dt > 0.02) spd = Math.max(0, (dist - prev.distance) / dt);
        }
        if ((!Number.isFinite(accelMag) || accelMag < 0) && prev && Number.isFinite(spd) && Number.isFinite(prev.speed)) {
          const dt = (now - prev.t) / 1000;
          if (dt > 0.02) accelMag = (spd - prev.speed) / dt;
        }
        prevRef.current = { t: now, speed: spd, distance: dist };

        setLive({ speed: Number.isFinite(spd) ? spd : 0, accel: Number.isFinite(accelMag) ? accelMag : 0, distance: Number.isFinite(dist) ? dist : 0 });
        setReadings(prevArr => {
        ...
        if (fin && (fin.finished === true || fin.finished === 1)) {
          ...
          setEndTime(ended);
          const summary = computeMetrics({ readings: [...(readings || []), { t: now, ...car, speed: spd, accel: accelMag, distance: dist }], trackDistance: Number(trackDistance) || 0, startTime: startTime || now, endTime: ended });
