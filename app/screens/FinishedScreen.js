...
export default function FinishedScreen({ navigation }) {
  ...
  const summary = lastSummary ?? computeMetrics({ readings, trackDistance: Number(trackDistance) || 0, startTime, endTime });
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, gap: 18, paddingBottom: 40 }}>
        ...
        <View style={{ borderWidth: 1, borderColor: '#eee', borderRadius: 16, padding: 16, gap: 14 }}>
        ...
        <Pressable onPress={() => { resetLap(); setLastSummary(null); navigation.popToTop(); }}
...
function Row({ label, value }) { return (<View style={{ flexDirection:'row', justifyContent:'space-between' }}><Text style={{ color:'#666' }}>{label}</Text><Text style={{ fontWeight:'700' }}>{value}</Text></View>); }
function Big({ label, value }) { return (<View><Text style={{ color:'#666', marginBottom:4 }}>{label}</Text><Text style={{ fontSize:40, fontWeight:'900', letterSpacing:0.5 }}>{value}</Text></View>); }
