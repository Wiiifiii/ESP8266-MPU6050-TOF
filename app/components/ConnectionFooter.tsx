...
function Dot({ status }: { status: 'online' | 'stale' | 'offline' }) {
  ...
  return (
    <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: bg, marginRight: 6 }} />
  );
}
    ...
    >
      <View style={stackStyle}>
        ...
        <Pill
          ...
          age={age(finish)}
          latency={finish.latencyMs}
        />
        <TouchableOpacity
          onPress={refreshOnce}
          style={{
            marginLeft: narrow ? 0 : 6,
            paddingHorizontal: 8,
            paddingVertical: 4,
            borderRadius: 12,
            backgroundColor: '#111827',
